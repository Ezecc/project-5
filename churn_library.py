...
def encoder_helper(dataframe, category_lst, response):
    ...
    '''
    # Copy DataFrmae
    cat_encoded_df = dataframe.copy(deep=True)
    ...
    for category in category_lst:
        ...
        if response:
            cat_encoded_df[category + '_' + response] = column_lst
        else:
            cat_encoded_df[category] = column_lst
...
def perform_feature_engineering(dataframe, response):
    ...
    # feature engineering
    cat_encoded_df = encoder_helper(
        dataframe=dataframe,
        category_lst=cat_columns,
        response=response)

    # target feature
    y = cat_encoded_df['Churn']
    ...
    # Features DataFrame
    X[keep_cols] = cat_encoded_df[keep_cols]
...
def train_models(X_train, X_test, y_train, y_test):
    ...
    # Perform grid search and fit the RandomForestClassifier
    ...
    # Perform train and test predictions for RandomForestClassifier
    y_train_preds_rf = cv_rfc.best_estimator_.predict(X_train)
    y_test_preds_rf = cv_rfc.best_estimator_.predict(X_test)

    # Perform train and test predictions for LogisticRegression
    ...
    # Generate trainig and testing result plots
...
if __name__ == '__main__':
    # Import data
    CUSTOMER_DF = import_data(data_path='./data/bank_data.csv')

    # Perform EDA
    EDA_DF = perform_eda(dataframe=CUSTOMER_DF)

    # Peroform feature engineering
